  ...
  | name :: others ->
      let _ =
        ...
        | Ok typelib -> (
            ...
            | None ->
                let strs =
                  [ "Unable to find"; name; "in the namespace"; namespace ]
                in
            ...
            | Some bi ->
                if Base_info.is_deprecated bi then
                  ...
                  let _ =
                    ...
                    | Bindings.Base_info.Boxed | Bindings.Base_info.Struct ->
                        let si = Struct_info.from_baseinfo bi in
                        if Struct_info.is_gtype_struct si then ()
                        else Bind_struct.parse_struct_info bi sources []
      ...
      in
      write_bindings_for namespace ?version dest_dir others
...
let write_constant_bindings_for namespace ?version sources skipped =
  ...
  | Ok typelib ->
      ...
      for i = 0 to n - 1 do
        ...
        | None -> ()
        | Some name -> (
            match (Base_info.is_deprecated bi, match_one_of name skipped) with
