...
let value_info_to_enum_type_conversion ocaml_type value =
  if ocaml_type = "Unsigned.uint32" then "Unsigned.UInt32.of_int " ^ value
    ...
    String.concat "" [ "Int32.of_int "; "("; value; ")" ]
...
let append_enum_of_value_fn enum_name ocaml_type values_and_variants sources =
  ...
  File.buff_add ml
    (String.concat "  else if v = "
       (List.map
          (fun (x, v) ->
            String.concat ""
              [
                value_info_to_enum_type_conversion ocaml_type x;
                " then ";
                v;
                "\n";
              ])
...
let append_enum_to_value_fn _enum_name ocaml_type values_and_variants sources =
  ...
  let branches =
    List.map
      (fun (x, v) ->
        String.concat ""
          [ v; " -> "; value_info_to_enum_type_conversion ocaml_type x; "\n" ])
      values_and_variants
    |> String.concat "  | "
  in
...
let get_values_and_variants info =
  ...
  let rec get_v_and_v i v_v =
    if i == n then List.rev v_v
      ...
      | Some value_info -> (
          let value_base_info = Value_info.to_baseinfo value_info in
          if Base_info.is_deprecated value_base_info then
            get_v_and_v (i + 1) v_v
          else
            match Base_info.get_name value_base_info with
            | None -> get_v_and_v (i + 1) v_v
            | Some const_name ->
                if Binding_utils.has_number_at_beginning const_name then
                  get_v_and_v (i + 1) v_v
                else
                  let value = Value_info.get_value value_info in
                  let variant_name = String.capitalize_ascii const_name in
                  get_v_and_v (i + 1)
                    ((Int64.to_string value, variant_name) :: v_v))
  in
  get_v_and_v 0 []
...
let append_ctypes_enum_bindings enum_name info sources =
  ...
  | Not_implemented tag_name ->
      let coms =
        Printf.sprintf "TODO enum %s : %s tag not implemented" enum_name
          tag_name
      in
...
let append_flags_list_to_value_fn _enum_name ocaml_type sources =
  ...
  let constant_type =
    if ocaml_type = "Unsigned.uint32" then "Unsigned.UInt32" else "Int32"
  in
...
let append_flags_view ctypes_typ sources =
  ...
  File.bprintf ml
    "let t_list_view = view ~read:list_of_value ~write:list_to_value %s"
    ctypes_typ
...
let append_ctypes_flags_bindings enum_name info sources =
  ...
  | Not_implemented tag_name ->
      let coms =
        Printf.sprintf "TODO flags %s : %s tag not implemented *)\n" enum_name
          tag_name
      in
      Sources.buffs_add_comments sources coms
  | Types { ocaml = ocaml_type; ctypes = ctypes_typ } ->
      ...
      append_enum_to_value_fn enum_name ocaml_type values_and_variants sources;
      Sources.buffs_add_eol sources;
      append_flags_list_of_value_fn enum_name ocaml_type values_and_variants
        sources;
      Sources.buffs_add_eol sources;
