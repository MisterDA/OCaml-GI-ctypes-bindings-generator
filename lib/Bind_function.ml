 ...
 * *)
let filter_same_argument_type_as_container container_name
...
let get_return_types callable container skip_types =
  if Callable_info.skip_return callable then Type_names [ ("unit", "void") ]
    ...
    let ret = Callable_info.get_return_type callable in
    let may_be_null = Callable_info.may_return_null callable in
    let can_throw_gerror = Callable_info.can_throw_gerror callable in
    match
      Binding_utils.type_info_to_bindings_types ret
        (may_be_null || can_throw_gerror)
    with
    ...
    | Types { ocaml = ocaml_type; ctypes = ctypes_typ } ->
        if Binding_utils.match_one_of ocaml_type skip_types then
          Skipped ocaml_type
        else if ocaml_type = "string" then
          Type_names [ ("string option", "string_opt") ]
        else
          let types =
            filter_same_argument_type_as_container container
              (ocaml_type, ctypes_typ)
          in
          Type_names [ types ]
...
let args_has search = function
  ...
  | Args args -> (
      ...
      | Some a -> Some a
      | None -> (
          match search args.out_list with
          | Some a -> Some a
...
let has_not_implemented_arg =
  let search_not_implemented =
    List.find_opt (fun a ->
        match a with Not_implemented _ -> true | _ -> false)
  in
  args_has search_not_implemented

let has_skipped_arg =
  let search_skipped =
    List.find_opt (fun a -> match a with Skipped _ -> true | _ -> false)
  in
  args_has search_skipped

let arg_may_be_null = function
  | Not_implemented message ->
      raise
        (Failure
           (Printf.sprintf "arg_may_be_null : Not_implemented -> %s" message))
  | Skipped message ->
      raise (Failure (Printf.sprintf "arg_may_be_null : Skipped -> %s" message))
...
let get_escaped_arg_name = function
  | Not_implemented message ->
      raise
        (Failure
           (Printf.sprintf "get_escaped_arg_names : Not_implemented -> %s"
              message))
  | Skipped message ->
      raise
        (Failure
           (Printf.sprintf "get_escaped_arg_names : Skipped -> %s" message))
  | Arg arg -> Binding_utils.ensure_valid_variable_name arg.name

(** Get a list of escaped names from a list of arguments. *)
let get_escaped_arg_names = List.map get_escaped_arg_name
...
let get_ocaml_type = function
  | Not_implemented message ->
      raise
        (Failure
           (Printf.sprintf "get_ocaml_type : Not_implemented -> %s" message))
  | Skipped message ->
      raise (Failure (Printf.sprintf "get_ocaml_type : Skipped -> %s" message))
  | Arg arg -> arg.ocaml_type

(** get the OCaml types of a list of arguments. *)
let get_ocaml_types = List.map (fun a -> get_ocaml_type a)

(** Transform a list of arguments to the corresponding mli signature. *)
let ocaml_types_to_mli_sig l = get_ocaml_types l |> String.concat " -> "
...
let get_ctypes_types = List.map (fun a -> get_ctypes_type a)

(** Transform a list of arguments to the corresponding ctypes foreign signature. *)
let ctypes_types_to_foreign_sig l = get_ctypes_types l |> String.concat " @-> "
...
let get_type_info = function
  | Not_implemented message ->
      raise
        (Failure
           (Printf.sprintf "get_type_info : Not_implemented -> %s" message))
  | Skipped message ->
      raise (Failure (Printf.sprintf "get_type_info : Skipped -> %s" message))
...
let get_args_information callable container skip_types =
  ...
  if n = 0 && not is_method then No_args
    ...
    let fetch_arg_info arg =
      ...
      | Types { ocaml = ot; ctypes = ct } ->
          let ocaml_type, ctypes_type =
            filter_same_argument_type_as_container cont_n (ot, ct)
          in
          if Binding_utils.match_one_of ocaml_type skip_types then
            Skipped ocaml_type
          else
            let name =
              ...
              | None ->
                  let msg =
                    "It should have a name :Bind_function Arg_info.in"
                  in
                  raise (Failure msg)
              | Some s -> s
            in
    ...
    let append_self_if_needed l =
      if is_method then
        let ocaml_type, ctypes_type =
          ...
          | None ->
              let msg = "The container can not be None, this is a Method" in
              raise (Failure msg)
          | Some (_, ot, ct) -> (ot, ct)
        in
        let arg =
    ...
    let rec _each_arg i { in_list; out_list; in_out_list } =
      if i >= n then
        {
          in_list = append_self_if_needed (List.rev in_list);
...
let return_gerror_result ?(indent = 1) ?(ret = "ret") () =
  ...
  Printf.sprintf
    "%smatch (!@ err_ptr_ptr) with\n\
     %s| None -> Ok %s\n\
     %s| Some _ -> let err_ptr = !@ err_ptr_ptr in\n\
    \  %slet _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) \
     err_ptr in\n\
    \  %sError (err_ptr)" sep sep ret sep sep sep
...
let write_mli_signature mli name arguments ocaml_ret can_throw_gerror =
  ...
  | Args args -> (
      ...
      in
      let ocaml_types_out = build_return_type_for_signature args.out_list in
      ...
      let args_in_mli_sig =
        match args.in_list with
        | [] -> None
        | l -> Some (ocaml_types_to_mli_sig l)
      in
      let ret_type =
        match ocaml_ret with "unit" -> None | _ -> Some ocaml_ret
      in
      if can_throw_gerror then
        match
          (args_in_mli_sig, ocaml_types_out, ocaml_types_in_out, ret_type)
        with
        | None, None, None, _ ->
            File.bprintf mli "unit -> (%s, %s) result\n" ocaml_ret
              error_ocaml_type
        ...
        | None, None, Some args_in_out, Some ocaml_ret ->
            File.bprintf mli "%s -> (%s * %s, %s) result\n" args_in_out_sig
              ocaml_ret args_in_out error_ocaml_type
        ...
        | None, Some args_out, Some args_in_out, Some ocaml_ret ->
            File.bprintf mli "%s -> (%s * %s * %s, %s) result\n" args_in_out_sig
              ocaml_ret args_out args_in_out error_ocaml_type
        ...
        match
          (args_in_mli_sig, ocaml_types_out, ocaml_types_in_out, ret_type)
        with
        | None, None, None, _ -> File.bprintf mli "unit -> %s\n" ocaml_ret
        | Some args_in, None, None, _ ->
        ...
        | None, None, Some args_in_out, Some ocaml_ret ->
            File.bprintf mli "%s -> (%s * %s)\n" args_in_out_sig ocaml_ret
              args_in_out
        | Some args_in, None, Some args_in_out, None ->
            File.bprintf mli "%s -> %s -> (%s)\n" args_in args_in_out_sig
              args_in_out
        | Some args_in, None, Some args_in_out, Some ocaml_ret ->
            File.bprintf mli "%s -> %s -> (%s * %s)\n" args_in args_in_out_sig
              ocaml_ret args_in_out
        | None, Some args_out, Some args_in_out, None ->
            File.bprintf mli "%s -> (%s * %s)\n" args_in_out_sig args_out
              args_in_out
        | None, Some args_out, Some args_in_out, Some ocaml_ret ->
            File.bprintf mli "%s -> (%s * %s * %s)\n" args_in_out_sig ocaml_ret
...
let write_function_name ml name arguments can_throw_gerror =
  ...
  match arguments with
  | No_args ->
      if can_throw_gerror then File.bprintf ml "let %s () =\n" name
      else File.bprintf ml "let %s =\n" name
  | Args args -> (
      ...
      match
        (args_in_names, args_out_names, args_in_out_names, can_throw_gerror)
      with
...
let write_foreign_declaration ml name symbol arguments can_throw_gerror
    ctypes_ret =
  let open Binding_utils in
  match arguments with
  | No_args ->
  ...
  | Args args -> (
      ...
      let args_by_ptr_to_foreign_sig = function
        ...
        | l ->
            Some
              (String.concat " @-> "
                 (List.map
                    (fun a -> Printf.sprintf "ptr (%s)" (get_ctypes_type a))
                    l))
      in
      ...
      let args_in_out_foreign_sig =
        args_by_ptr_to_foreign_sig args.in_out_list
      in
      if can_throw_gerror then
        let _ =
          File.bprintf ml "  let %s_raw =\n    foreign \"%s\" " name symbol
        in
        match
          (args_in_foreign_sig, args_out_foreign_sig, args_in_out_foreign_sig)
        with
        | None, None, None ->
            (* Should not appear raise an exception ? *)
            File.bprintf ml "(ptr (%s) @-> returning (%s))\n  in\n"
              error_ctypes_type ctypes_ret
...
let write_compute_value_instructions ml name arguments can_throw_gerror =
  ...
  | Args args -> (
      ...
      let args_ptr_names = function
        ...
        | l ->
            Some
              (List.map (fun a -> get_escaped_arg_name a ^ "_ptr") l
              |> String.concat " ")
      in
      let args_in_names = args_names args.in_list in
...
let write_allocation_instructions ml name arguments container can_throw_gerror =
  ...
  | Args args -> (
      let get_out_arg_alloc_instructions a =
        ...
        | None ->
            let message =
              "get_out_argument_allocation_intructions: no typeinfo for arg"
              ^ name'
            in
            let f = Failure message in
            raise f
        | Some type_info -> (
            ...
            | Error message ->
                let message' =
                  Printf.sprintf
                    "unable to get type to allocate for argument named %s of \
                     type '%s' in function %s"
                    name' message name
                in
                let f = Failure message' in
                raise f
            | Ok instructions ->
                let pattern = container ^ "." in
      ...
      let get_in_out_arg_alloc_instructions a =
        ...
        | None ->
            let message =
              "get_in_out_argument_allocation_intructions: no typeinfo for arg"
              ^ name'
            in
            let f = Failure message in
            raise f
        | Some type_info -> (
            let may_be_null = arg_may_be_null a in
            match
              allocate_out_argument_with_default_value type_info name'
                may_be_null name'
            with
            | Error message ->
                let message' =
      ...
      let args_out_alloc = function
        ...
        | l ->
            let instructions =
              List.map (fun a -> get_out_arg_alloc_instructions a) l
            in
      ...
      let args_in_out_alloc = function
        ...
        | l ->
            let instructions =
              List.map (fun a -> get_in_out_arg_alloc_instructions a) l
            in
            Some (String.concat "  " instructions)
      in
      match
        (args_out_alloc args.out_list, args_in_out_alloc args.in_out_list)
      with
      | None, None -> ()
      | Some args_out, None -> File.bprintf ml "  %s" args_out
      | None, Some args_in_out -> File.bprintf ml "  %s" args_in_out
      | Some args_out, Some args_in_out ->
          File.bprintf ml "  %s  %s" args_out args_in_out)

let write_build_return_value_instructions ml name arguments can_throw_gerror
    ocaml_ret =
  let open Binding_utils in
  match arguments with
  | No_args ->
      if can_throw_gerror then File.buff_add_line ml (return_gerror_result ())
  | Args args -> (
      let write_get_value_from_pointer_instructions a =
        ...
        | None ->
            raise
              (Failure
                 ("write_get_value_from_pointer_instructions: no typeinfo for \
                   arg " ^ name'))
        | Some type_info -> (
            let may_be_null = arg_may_be_null a in
            ...
            | Error message ->
                let message' =
                  Printf.sprintf
                    "unable to get instructions to get value for argument \
                     named %s of type '%s' in function %s"
                    name' message name
                in
      ...
      let get_values_from_ptr_instructions args_list sep =
        ...
        | [] -> None
        | l ->
            let f a = write_get_value_from_pointer_instructions a in
            let l' = List.map f l in
            Some (String.concat sep l')
      in
      let get_args_out_values =
        get_values_from_ptr_instructions args.out_list
      in
      let get_args_in_out_values =
        get_values_from_ptr_instructions args.in_out_list
      in
      let args_out_tuple = escaped_arg_names_to_tuple_form args.out_list in
      let args_in_out_tuple =
        escaped_arg_names_to_tuple_form args.in_out_list
      in
      if can_throw_gerror then
...
let generate_callable_bindings callable name container symbol arguments
    ret_types sources =
  let open Binding_utils in
  ...
  let can_throw_gerror = Callable_info.can_throw_gerror callable in
  let ocaml_ret' =
    if ocaml_ret = "string" then "string option" else ocaml_ret
  in
  let ctypes_ret' =
    if ctypes_ret = "string" then "string_opt" else ctypes_ret
  in
  write_mli_signature mli name arguments ocaml_ret' can_throw_gerror;
  write_function_name ml name arguments can_throw_gerror;
  write_foreign_declaration ml name symbol arguments can_throw_gerror
    ctypes_ret';
  ...
  write_build_return_value_instructions ml name arguments can_throw_gerror
    ocaml_ret
...
let should_be_implemented args sources symbol =
  ...
  let get_info_for_non_usable_arg = function
    | Arg _ ->
        let msg =
          "get_info_for_non_usable_arg: this should never has been reached"
        in
        raise (Failure msg)
  ...
  | Some arg, _ ->
      ...
      let coms =
        Printf.sprintf "Not implemented %s type %s not implemented" symbol i
      in
      let _ = Sources.buffs_add_comments sources coms in
      false
  | None, None -> true
  | None, Some arg ->
      let i = get_info_for_non_usable_arg arg in
      let coms = Printf.sprintf " %s type %s skipped" symbol i in
      let _ = Sources.buffs_add_skipped sources coms in
      false
...
let append_ctypes_function_bindings raw_name info container sources skip_types =
  ...
  if should_be_implemented args sources s then
    match get_return_types ci cn skip_types with
    | Not_handled t ->
        let coms =
          Printf.sprintf "Not implemented %s return type %s not handled" s t
        in
    ...
    | Type_names rt -> generate_callable_bindings ci n cn s args rt sources
