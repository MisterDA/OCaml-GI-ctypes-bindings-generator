...
module Option = struct
  let value ~default value_or_none =
    match value_or_none with None -> default | Some v -> v
end
...
let escape_OCaml_keywords variable_name =
  ...
  | "lxor" | "match" | "method" | "method!" | "mod" | "module" | "mutable"
  | "nonrec" | "object" | "of" | "open" | "open!" | "or" | "private" | "rec"
...
let escape_OCaml_types name =
  match name with
  | "int" | "float" | "list" | "string" | "char" | "array" -> "_" ^ name
  | _ -> name

(* Taken from https://realworldocaml.org/v1/en/html/foreign-function-interface.html. *)
let escape_Ctypes_types name =
  match name with
  | "void" | "char" | "schar" | "short" | "int" | "long" | "llong" | "nativeint"
  | "int8_t" | "int16_t" | "int32_t" | "int64_t" | "uchar" | "uint8_t"
  | "uint16_t" | "uint32_t" | "uint64_t" | "size_t" | "ushort" | "uint"
  | "ulong" | "ullong" | "float" | "double" | "complex32" | "complex64" ->
      "_" ^ name
  | _ -> name
...
let ensure_valid_variable_name name =
  escape_OCaml_keywords name |> escape_OCaml_types |> escape_Ctypes_types
  |> escape_number_at_beginning |> escape_new_pattern
...
let match_one_of str patterns =
  ...
  let rec iterate = function
    | [] -> false
    | hd :: tl -> if found hd then true else iterate tl
  in
  iterate patterns
...
module File = struct
  ...
  let add_open_ctypes t = write_open_module t "Ctypes"
  let add_open_foreign t = write_open_module t "Foreign"
  let add_empty_line t = Printf.fprintf t.descr "%s" "\n"

  let add_comments t information =
    Printf.fprintf t.descr "(* %s. *)\n" information

  let buff_add t str = Buffer.add_string t.buffer str
  ...
  let buff_add_eol t = Buffer.add_string t.buffer "\n"
  let bprintf t = Printf.bprintf t.buffer
...
let type_tag_to_bindings_types = function
  ...
  | Bindings.Types.Uint8 ->
      Types { ocaml = "Unsigned.uint8"; ctypes = "uint8_t" }
  | Bindings.Types.Int16 -> Types { ocaml = "int"; ctypes = "int16_t" }
  | Bindings.Types.Uint16 ->
      Types { ocaml = "Unsigned.uint16"; ctypes = "uint16_t" }
  | Bindings.Types.Int32 -> Types { ocaml = "int32"; ctypes = "int32_t" }
  | Bindings.Types.Uint32 ->
      Types { ocaml = "Unsigned.uint32"; ctypes = "uint32_t" }
  | Bindings.Types.Int64 -> Types { ocaml = "int64"; ctypes = "int64_t" }
  | Bindings.Types.Uint64 ->
      Types { ocaml = "Unsigned.uint64"; ctypes = "uint64_t" }
...
let check_if_pointer type_info maybe_null (ocaml_t, ctypes_t) =
  if Type_info.is_pointer type_info then
    if maybe_null then
      { ocaml = ocaml_t ^ " ptr option"; ctypes = "ptr_opt " ^ ctypes_t }
    else { ocaml = ocaml_t ^ " ptr"; ctypes = "ptr " ^ ctypes_t }
...
let interface_to_binding_types interface check_if_pointer type_info =
  ...
  | Enum as t ->
      let suffixes =
        if Type_info.is_pointer type_info then (".t", ".t_view")
        else (".t", ".t_view")
      in
      interface_bindings t suffixes
  ...
  | ( Boxed | Invalid | Function | Callback | Interface | Constant | Invalid_0
    ...
    | Unresolved ) as t ->
      Not_implemented (Bindings.Base_info.string_of_info_type t)
...
let get_data_types_of_container type_info =
  ...
  | ( Bindings.Types.Array | Bindings.Types.GList | Bindings.Types.GSList
    | Bindings.Types.GHash | Bindings.Types.Error | Bindings.Types.Unichar
...
let type_info_to_bindings_types type_info maybe_null =
  ...
  let get_full_types_of_container suffix_type =
    ...
    in
    let container_type =
      String.concat " " [ suffix_type ^ ".t structure"; "(*"; data_type; "*)" ]
    in
  ...
  | None -> (
      ...
      | Bindings.Types.Uint8 ->
          Types (check_if_pointer ("Unsigned.uint8", "uint8_t"))
      | Bindings.Types.Int16 -> Types (check_if_pointer ("int", "int16_t"))
      | Bindings.Types.Uint16 ->
          Types (check_if_pointer ("Unsigned.uint16", "uint16_t"))
      | Bindings.Types.Int32 -> Types (check_if_pointer ("int32", "int32_t"))
      | Bindings.Types.Uint32 ->
          Types (check_if_pointer ("Unsigned.uint32", "uint32_t"))
      | Bindings.Types.Int64 -> Types (check_if_pointer ("int64", "int64_t"))
      | Bindings.Types.Uint64 ->
          Types (check_if_pointer ("Unsigned.uint64", "uint64_t"))
      ...
      | Bindings.Types.Array -> (
          ...
          | Some array_type -> (
              ...
              | Bindings.Types.C ->
                  Not_implemented "C Array type for Types.Array tag"
              | Bindings.Types.Array ->
                  Types (check_if_pointer ("Array.t structure", "Array.t_typ"))
              | Bindings.Types.Ptr_array ->
                  Types
                    (check_if_pointer
                       ("Ptr_array.t structure", "Ptr_array.t_typ"))
              | Bindings.Types.Byte_array ->
                  Types
                    (check_if_pointer
                       ("Byte_array.t structure", "Byte_array.t_typ"))))
      ...
      | ( Bindings.Types.GType | Bindings.Types.Unichar
        | Bindings.Types.Interface ) as tag ->
          Not_implemented (Bindings.Types.string_of_tag tag))
  | Some interface ->
...
let allocate_out_argument type_info var_name maybe_null =
  ...
  | None -> (
      match Type_info.get_tag type_info with
      | Bindings.Types.Void ->
          check_if_pointer ("void", "None") |> _allocate_simple_instructions
      | Bindings.Types.Boolean ->
          check_if_pointer ("bool", "false") |> _allocate_simple_instructions
      | Bindings.Types.Int8 ->
          check_if_pointer ("int8_t", "0") |> _allocate_simple_instructions
      | Bindings.Types.Uint8 ->
          check_if_pointer ("uint8_t", "Unsigned.UInt8.zero")
          |> _allocate_simple_instructions
      | Bindings.Types.Int16 ->
          check_if_pointer ("int16_t", "0") |> _allocate_simple_instructions
      | Bindings.Types.Uint16 ->
          check_if_pointer ("uint16_t", "Unsigned.UInt16.zero")
          |> _allocate_simple_instructions
      | Bindings.Types.Int32 ->
          check_if_pointer ("int32_t", "Int32.zero")
          |> _allocate_simple_instructions
      | Bindings.Types.Uint32 ->
          check_if_pointer ("uint32_t", "Unsigned.UInt32.zero")
          |> _allocate_simple_instructions
      | Bindings.Types.Int64 ->
          check_if_pointer ("int64_t", "Int64.zero")
          |> _allocate_simple_instructions
      | Bindings.Types.Uint64 ->
          check_if_pointer ("uint64_t", "Unsigned.UInt64.zero")
          |> _allocate_simple_instructions
      | Bindings.Types.Float ->
          check_if_pointer ("float", "0.0") |> _allocate_simple_instructions
      | Bindings.Types.Double ->
          check_if_pointer ("double", "0.0") |> _allocate_simple_instructions
      | Bindings.Types.GType ->
          check_if_pointer ("int64_t", "Int64.zero")
          |> _allocate_simple_instructions
      | Bindings.Types.Utf8 | Bindings.Types.Filename ->
      ...
      | Bindings.Types.GList ->
          check_if_pointer ("List.t_typ", "None")
          |> _allocate_simple_instructions
      | Bindings.Types.GSList ->
          check_if_pointer ("SList.t_typ", "None")
          |> _allocate_simple_instructions
      | Bindings.Types.GHash ->
          check_if_pointer ("Hash_table.t_typ", "None")
          |> _allocate_simple_instructions
      | Bindings.Types.Error ->
          check_if_pointer ("Error.t_typ", "None")
          |> _allocate_simple_instructions
  ...
  | Some interface -> (
      ...
      | Bindings.Base_info.Struct -> (
          match get_binding_name interface with
          | None ->
              Error (Printf.sprintf "%s interface struct without name" var_name)
          | Some name ->
              check_if_pointer
                ( Printf.sprintf "%s.t_typ" name,
      ...
      | Bindings.Base_info.Enum | Bindings.Base_info.Flags -> (
          match get_binding_name interface with
          | None ->
              Error (Printf.sprintf "%s interface enum without name" var_name)
          | Some name -> (
              let enum_info = Enum_info.from_baseinfo interface in
              ...
              | Bindings.Types.Int32 ->
                  _allocate_simple_instructions
                    (view_name, def_val_constructor "Int32.zero")
              | Bindings.Types.Uint32 ->
                  _allocate_simple_instructions
                    (view_name, def_val_constructor "Unsigned.UInt32.zero")
              | _ ->
                  Error
                    (Printf.sprintf
                       "%s interface enum %s with a bad storage type" var_name
                       name)))
      | Bindings.Base_info.Object -> (
          match get_binding_name interface with
          | None ->
              Error (Printf.sprintf "%s interface object without name" var_name)
          | Some name ->
              check_if_pointer
                ( Printf.sprintf "%s.t_typ" name,
                  Printf.sprintf "(coerce (void ptr) %s.t_typ null)" name )
              |> _allocate_simple_instructions)
      | Bindings.Base_info.Invalid as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Function as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Callback as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Boxed as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Interface as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Constant as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Invalid_0 as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Union as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Value as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Signal as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Vfunc as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Property as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Field as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Arg as t ->
          Error (Bindings.Base_info.string_of_info_type t)
      | Bindings.Base_info.Unresolved as t ->
          Error (Bindings.Base_info.string_of_info_type t))

let allocate_out_argument_with_default_value type_info var_name maybe_null
    default =
  let check_if_pointer (ctypes_t, default_value) =
    (* Consider that a pointer can always be null no need to check for maybe_null. *)
    if Type_info.is_pointer type_info then
  ...
  | None -> (
      match Type_info.get_tag type_info with
      | Bindings.Types.Void ->
          check_if_pointer ("void", default) |> _allocate_simple_instructions
      | Bindings.Types.Boolean ->
          check_if_pointer ("bool", default) |> _allocate_simple_instructions
      | Bindings.Types.Int8 ->
          check_if_pointer ("int8_t", default) |> _allocate_simple_instructions
      | Bindings.Types.Uint8 ->
          check_if_pointer ("uint8_t", default) |> _allocate_simple_instructions
      | Bindings.Types.Int16 ->
          check_if_pointer ("int16_t", default) |> _allocate_simple_instructions
      | Bindings.Types.Uint16 ->
          check_if_pointer ("uint16_t", default)
          |> _allocate_simple_instructions
      | Bindings.Types.Int32 ->
          check_if_pointer ("int32_t", default) |> _allocate_simple_instructions
      | Bindings.Types.Uint32 ->
          check_if_pointer ("uint32_t", default)
          |> _allocate_simple_instructions
      | Bindings.Types.Int64 ->
          check_if_pointer ("int64_t", default) |> _allocate_simple_instructions
      | Bindings.Types.Uint64 ->
          check_if_pointer ("uint64_t", default)
          |> _allocate_simple_instructions
      | Bindings.Types.Float ->
          check_if_pointer ("float", default) |> _allocate_simple_instructions
      | Bindings.Types.Double ->
          check_if_pointer ("double", default) |> _allocate_simple_instructions
      | Bindings.Types.GType ->
          check_if_pointer ("int64_t", default) |> _allocate_simple_instructions
      | Bindings.Types.Utf8 | Bindings.Types.Filename ->
      ...
      | Bindings.Types.Interface as t -> Error (Bindings.Types.string_of_tag t)
      | Bindings.Types.GList ->
          check_if_pointer ("List.t_typ", default)
          |> _allocate_simple_instructions
      | Bindings.Types.GSList ->
          check_if_pointer ("SList.t_typ", default)
          |> _allocate_simple_instructions
      | Bindings.Types.GHash ->
          check_if_pointer ("Hash_table.t_typ", default)
          |> _allocate_simple_instructions
      | Bindings.Types.Error ->
          check_if_pointer ("Error.t_typ", default)
          |> _allocate_simple_instructions
      | Bindings.Types.Unichar as t -> Error (Bindings.Types.string_of_tag t))
  | Some interface -> (
      match Base_info.get_type interface with
      | Bindings.Base_info.Struct -> (
          ...
          | None ->
              Error (Printf.sprintf "%s interface struct without name" var_name)
          | Some name ->
              check_if_pointer (Printf.sprintf "%s.t_typ" name, default)
              |> _allocate_simple_instructions)
      | Bindings.Base_info.Type ->
          check_if_pointer ("int64_t", default) |> _allocate_simple_instructions
      | Bindings.Base_info.Enum | Bindings.Base_info.Flags -> (
          ...
          | Some name -> (
              ...
              match Enum_info.get_storage_type enum_info with
              | Bindings.Types.Int32 ->
                  _allocate_simple_instructions (view_name, default)
              | Bindings.Types.Uint32 ->
                  _allocate_simple_instructions (view_name, default)
              | _ ->
                  Error
...
let get_out_argument_value type_info var_name _maybe_null =
  ...
  | None -> (
      match Type_info.get_tag type_info with
      | Bindings.Types.Void | Bindings.Types.Boolean | Bindings.Types.Int8
      | Bindings.Types.Uint8 | Bindings.Types.Int16 | Bindings.Types.Uint16
      | Bindings.Types.Int32 | Bindings.Types.Uint32 | Bindings.Types.Int64
      | Bindings.Types.Uint64 | Bindings.Types.Float | Bindings.Types.Double
      | Bindings.Types.GType | Bindings.Types.Utf8 | Bindings.Types.Filename
      | Bindings.Types.Array | Bindings.Types.Interface | Bindings.Types.GList
      | Bindings.Types.GSList | Bindings.Types.GHash | Bindings.Types.Error ->
          _get_value_simple_instructions ()
      | Bindings.Types.Unichar as t -> Error (Bindings.Types.string_of_tag t))
  | Some interface -> (
      match Base_info.get_type interface with
      | Bindings.Base_info.Struct -> (
