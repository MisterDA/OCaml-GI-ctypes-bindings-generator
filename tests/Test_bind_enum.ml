...
let get_enum_info namespace enum_name =
  ...
  | Some base_info -> (
      ...
      | Bindings.Base_info.Enum ->
          let enum_info = Enum_info.from_baseinfo base_info in
          Some enum_info
      | _ -> None)
...
let test_append_enum_of_value_fn test_ctxt =
  ...
  in
  enum_test namespace name (fun info ->
      test_writing test_ctxt info name writer enum_type_of_value_sig
        enum_type_of_value)
...
let test_append_enum_to_value_fn test_ctxt =
  ...
  in
  enum_test namespace name (fun info ->
      test_writing test_ctxt info name writer enum_type_to_value_sig
        enum_type_to_value)
...
let flags_type_list_to_value_sig =
  "val list_to_value:\n  t_list -> Unsigned.uint32"

let flags_type_list_to_value =
...
let flags_type_list_of_value_sig =
  "val list_of_value:\n  Unsigned.uint32 -> t_list"

let flags_type_list_of_value =
  "let list_of_value v =\n\
  \  let open Unsigned.UInt32 in\n\
  \  let all_flags = [( 0 , None ); ( 1 , Keep_comments ); ( 2 , \
   Keep_translations )]\n\
  \  in\n\
  \  let rec build_flags_list allf acc =\n\
  \    match allf with\n\
  \    | [] -> acc\n\
  \    | (i, f) :: q -> if ((logand v (of_int i )) <> zero) then \
   build_flags_list q (f :: acc)\n\
...
let test_append_enum_flags_list_to_value_fn test_ctxt =
  ...
  in
  flags_test namespace name (fun info ->
      test_writing test_ctxt info name writer flags_type_list_to_value_sig
        flags_type_list_to_value)
...
let test_append_enum_flags_list_of_value_fn test_ctxt =
  ...
  in
  flags_test namespace name (fun info ->
      test_writing test_ctxt info name writer flags_type_list_of_value_sig
        flags_type_list_of_value)
...
let test_append_flags_view test_ctxt =
  ...
  in
  enum_test namespace name (fun info ->
      test_writing test_ctxt info name writer flags_type_view_sig
        flags_type_view)
...
let tests =
  ...
  >::: [
         ...
         "Bind_enum append enum flags of value"
         >:: test_append_enum_flags_of_value_fn;
         "Bind_enum append enum flags to value"
         >:: test_append_enum_flags_to_value_fn;
         "Bind_enum append enum flags list to value"
         >:: test_append_enum_flags_list_to_value_fn;
         "Bind_enum append enum flags list of value"
         >:: test_append_enum_flags_list_of_value_fn;
       ]
