...
let test_get_return_types _test_ctx =
  test_function_info name (fun info ->
      ...
      | Type_names types ->
          let ocaml_type, ctypes_type = List.hd types in
          assert_equal_string "int32" ocaml_type;
          assert_equal_string "int32_t" ctypes_type
      | _ ->
          assert_equal_string "It should returns "
            "the return value ocaml and ctypes types")
...
let test_escape_bad_function_name test_ctxt =
  ...
  | Some i ->
      let si = Struct_info.from_baseinfo i in
...
let test_function_bindings_function_no_args_throw_gerror test_ctxt =
  ...
  | Some info ->
      ...
      let ml =
        "let clear_error () =\n\
        \  let clear_error_raw =\n\
        \    foreign \"g_clear_error\" (ptr (ptr_opt Error.t_typ) @-> \
         returning (void))\n\
        \  in\n\
      ...
      let writer n i srcs =
        let _ =
          Bind_function.append_ctypes_function_bindings n i None srcs []
        in
        Binding_utils.Sources.write_buffs srcs
      in
...
let test_function_bindings_for_in_args_only_function test_ctxt =
  ...
  | Some info ->
      ...
      let ml =
        ...
        \  foreign \"g_date_get_sunday_weeks_in_year\" (uint16_t @-> returning \
         (uint8_t))"
      in
      let writer n i srcs =
        let _ =
...
let test_function_bindings_for_in_args_only_function_gerror test_ctxt =
  ...
  | Some info ->
      ...
      let mli =
        "val dir_make_tmp :\n\
        \  string option -> (string option, Error.t structure ptr option) \
         result"
      in
      let ml =
        "let dir_make_tmp tmpl =\n\
        \  let dir_make_tmp_raw =\n\
        \    foreign \"g_dir_make_tmp\" (string_opt @-> ptr (ptr_opt \
         Error.t_typ) @-> returning (string_opt))\n\
        \  in\n\
        \  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in\n\
  ...
  | Some date_time_info -> (
      ...
      | Some mi ->
          ...
          let writer n i srcs =
            let c = Some (cont, "t structure ptr", "ptr t_typ") in
            let _ =
              Bind_function.append_ctypes_function_bindings n i c srcs []
            in
            Binding_utils.Sources.write_buffs srcs
          in
...
let test_function_bindings_for_args_out_as_enum_function test_ctxt =
  ...
  | Some bi -> (
      ...
      | Bindings.Base_info.Object -> (
          ...
          | None ->
              assert_equal_string name "No base info found for this method"
          | Some info ->
              let mli =
                "val get_attribute_data :\n\
                \  t -> string -> (bool * File_attribute_type.t * unit ptr * \
                 File_attribute_status.t)"
              in
              let ml =
                "let get_attribute_data self attribute =\n\
                \  let get_attribute_data_raw =\n\
                \    foreign \"g_file_info_get_attribute_data\" (t_typ @-> \
                 string @-> ptr (File_attribute_type.t_view) @-> ptr (ptr \
                 void) @-> ptr (File_attribute_status.t_view) @-> returning \
                 (bool))\n\
                \  in\n\
                \  let _type_ptr = allocate File_attribute_type.t_view \
                 (File_attribute_type.t_view.of_value (Unsigned.UInt32.zero)) \
                 in\n\
                \  let value_pp_ptr = allocate (ptr_opt void) None in\n\
                \  let status_ptr = allocate File_attribute_status.t_view \
                 (File_attribute_status.t_view.of_value \
                 (Unsigned.UInt32.zero)) in\n\
                \  let ret = get_attribute_data_raw self attribute _type_ptr \
                 value_pp_ptr status_ptr in\n\
...
let test_function_bindings_for_args_out_with_gerror_function test_ctxt =
  ...
  | Some info ->
      ...
      let mli =
        ...
        \  string -> (string option * string option, Error.t structure ptr \
         option) result"
      in
      let ml =
...
let tests =
  ...
  >::: [
         ...
         "Bind_function escape bad function name"
         >:: test_escape_bad_function_name;
         "Bind_function test function bindings for function with no args that \
          throws gerror"
         >:: test_function_bindings_function_no_args_throw_gerror;
         "Bind_function test function bindings for in args only function"
         >:: test_function_bindings_for_in_args_only_function;
         "Bind_function test function bindings for args out function"
         >:: test_function_bindings_for_args_out_function;
         "Bind_function test function bindings for args out as enum function"
         >:: test_function_bindings_for_args_out_as_enum_function;
         "Bind_funtcion test function bindings for in args only function that \
          can throw GError"
         >:: test_function_bindings_for_in_args_only_function_gerror;
         "Bind_function test function bindings for args out with gerror \
          function" >:: test_function_bindings_for_args_out_with_gerror_function;
       ]
